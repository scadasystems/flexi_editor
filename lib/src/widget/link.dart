  ...
  Widget build(BuildContext context) {
    ...
    return Listener(
      ...
      child: GestureDetector(
        ...
        onScaleStart: (details) => policy.onLinkScaleStart(linkData.id, details),
        onScaleUpdate: (details) => policy.onLinkScaleUpdate(linkData.id, details),
        onScaleEnd: (details) => policy.onLinkScaleEnd(linkData.id, details),
        onLongPress: () => policy.onLinkLongPress(linkData.id),
        onLongPressStart: (details) => policy.onLinkLongPressStart(linkData.id, details),
        onLongPressMoveUpdate: (details) => policy.onLinkLongPressMoveUpdate(linkData.id, details),
        onLongPressEnd: (details) => policy.onLinkLongPressEnd(linkData.id, details),
              ...
              ...linkData.linkPoints.getRange(1, linkData.linkPoints.length - 1).map(
                (jointPoint) {
                  ...
                  return GestureDetector(
                    ...
                    onTapUp: (details) => policy.onLinkJointTapUp(index, linkData.id, details),
                    onTapCancel: () => policy.onLinkJointTapCancel(index, linkData.id),
                    onScaleStart: (details) => policy.onLinkJointScaleStart(index, linkData.id, details),
                    onScaleUpdate: (details) => policy.onLinkJointScaleUpdate(index, linkData.id, details),
                    onScaleEnd: (details) => policy.onLinkJointScaleEnd(index, linkData.id, details),
                    onLongPress: () => policy.onLinkJointLongPress(index, linkData.id),
                    onLongPressStart: (details) => policy.onLinkJointLongPressStart(index, linkData.id, details),
                    onLongPressMoveUpdate: (details) =>
                        policy.onLinkJointLongPressMoveUpdate(index, linkData.id, details),
                    onLongPressEnd: (details) => policy.onLinkJointLongPressEnd(index, linkData.id, details),
